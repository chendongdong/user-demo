<template>
  <div class="yimi-service">
    <div class="app-header">
      <x-header :left-options="{showBack: true, backText: '返回'}" v-if="groupInfo">
        {{groupInfo.group_name}}({{groupInfo.user_list.length}})
        <img src="~assets/image/ic_group_info.png" slot="right" @click="jump2GroupInfo" class="right">
      </x-header>
    </div>
...
//          this.suggestionVisible = false
    ...
    methods: {
      jump2GroupInfo () {
        this.$router.push({name: 'GroupInfo', params: {groupId: this.groupId}})
      },
      select (item) {
        this.suggestionVisible = false
        let question = {
...
//        if (!divMsg || !divMsg.getElementsByTagName) return
      ...
      initWebSocket () {
        // ws地址
        const wsuri = this.$store.state.webSocketServer + 'gchat?userid=' + this.userId + '&username=' +
          this.userName + '&usertype=' + this.userType + '&groupid=' + this.groupId
...
//        console.log('redata=', redata)
      ...
      websocketsend (sendMsg) {
        let message = {
          from_identity: this.userId + '|' + this.userName + '|' + this.userType + '|' + this.groupId,
          to_identity: this.groupId + '|group|4',
          content: sendMsg,
          img: this.userHead
        }
    ...
    mounted () {
      this.$nextTick(function () {
        ...
        this.$store.dispatch('groupHisMsg', {group_id: this.groupId})
    ...
    beforeDestroy () {
      ...
      this.$store.dispatch('setGroupChatMsg', temp)
    }
  }
</script>
<style lang="scss">
